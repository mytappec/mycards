...
<style>
  ...
  *{box-sizing:border-box;}
  body{margin:0;min-height:100vh;background:var(--page-bg);font-family:'Quicksand','Segoe UI',sans-serif;padding:18px 14px;}
  ...
  .reward-note{margin-top:14px;background:var(--butter-mid);border:2px solid var(--border-reward);border-radius:16px;padding:10px 14px;display:flex;align-items:center;gap:10px;color:var(--reward-body);font-size:12.5px;}
  .reward-note .r-emoji{font-size:25px;flex-shrink:0;}
  .reward-note strong{display:block;font-family:var(--font-display);font-weight:var(--font-weight-reward);font-style:var(--font-style-reward);font-size:14px;margin-bottom:1px;color:var(--reward-heading);}
  ...
  .mascot-float{position:absolute;right:-10px;bottom:-16px;width:76px;height:auto;pointer-events:none;filter:drop-shadow(0 6px 10px rgba(0,0,0,.18));animation:mascotFloat 3.4s ease-in-out infinite;z-index:6;}
  @keyframes mascotFloat{0%,100%{transform:translateY(0) rotate(-3deg);}50%{transform:translateY(-9px) rotate(3deg);}}
  @media (max-width:380px){ .mascot-float{width:58px;right:-4px;bottom:-10px;} }
  .footer-brand{text-align:center;margin:14px 0 0;}
  .footer-brand img{height:24px;width:auto;margin:0 auto 4px;display:block;}
  .footer-brand p{font-size:12px;color:var(--text-credit);margin:0;}
